...
class LocalLLM:
    def __init__(
        ...
        Args:
            ...
            n_batch: Batch size for prompt processing (higher = faster)
    ...
    def _load_model(self):
        ...
        try:
            self.model = Llama(
                ...
                use_mmap=True,  # Memory-mapped file access for faster load
                verbose=False,
            )
    ...
    def generate_stream(self, prompt: str, max_tokens: int = 512, temperature: float = 0.7) -> Iterator[str]:
        ...
        try:
            ...
            self.last_tokens_per_second = (
                token_count / self.last_inference_time if self.last_inference_time > 0 else 0.0
            )
    ...
    def _build_chat_prompt(self, message: str, chat_history: list | None = None) -> str:
        """
