...
JSON Output:"""
    ...
    def _looks_like_json(self, text: str) -> bool:
        ...
        return not ("{" in temp and ":" not in temp)

    def _clean_json_response(self, response: str) -> str | None:
        """
        ...
        start = (
            min(first_bracket, first_brace)
            if first_bracket != -1 and first_brace != -1
            else max(first_bracket, first_brace)
        )
    ...
    def extract_single_memory(
        self, text: str, memory_type: str, source_document: str | None = None
    ) -> MemorySchema | None:
        """
